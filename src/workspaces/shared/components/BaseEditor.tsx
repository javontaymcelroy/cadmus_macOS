...
export function BaseEditor({ toolbar, placeholder = 'Start writing...', additionalExtensions = [], emptyMessage = 'Select a document to start editing' }: BaseEditorProps) {
  const { 
    currentProject,
    activeDocumentId, 
    documents,
  ...
  const { documentTitle, titleFontFamily } = useMemo(() => {
    ...
    return {
      documentTitle: h1Info.text || activeDoc?.title || 'Untitled Document',
  ...
  const extensions = useMemo(() => [
    ...
    Link.configure({
      openOnClick: false,
      HTMLAttributes: {
        class: 'text-gold-400 underline cursor-pointer hover:text-gold-300',
      },
    }),
    UniqueID.configure({
  ...
  ], [placeholder, additionalExtensions])
    ...
    editorProps: {
      ...
      handlePaste: (view, event, _slice) => {
        ...
        try {
          ...
          if (citationMark) {
            // Insert pasted text as regular text, then append a citation chip
            const plainText = schema.text(citationData.text)
            const chipText = schema.text('Source', [citationMark])
            const { from, to } = tr.selection
            tr.replaceWith(from, to, [plainText, chipText])
            view.dispatch(tr)
            return true
          }
        } catch (err) {
  ...
  if (!activeDocumentId || !activeDoc) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-theme-muted font-ui">{emptyMessage}</p>
      </div>
    )
  }
